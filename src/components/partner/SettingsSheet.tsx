...
import { Globe, Lock, LogOut, Trash2, Loader2, ChevronRight, CreditCard } from "lucide-react";
...
const SettingsSheet = ({ open, onOpenChange }: SettingsSheetProps) => {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (!session) {
        toast({ title: t("common.error"), description: t("errors.noActiveSession"), variant: "destructive" });
        return;
      }
      ...
      if (error) {
        ...
        toast({ title: t("common.error"), description: t("errors.deleteAccountFailed"), variant: "destructive" });
        return;
      }
      toast({ title: t("common.success"), description: t("success.accountDeleted") });
      await supabase.auth.signOut();
      navigate("/login");
    } catch (error) {
      console.error("Delete account error:", error);
  ...
  return (
    <>
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent
          className="max-h-[90vh] border-t-0 bg-gradient-to-b from-sky-50/40 via-white to-white"
        ...
        >
          ...
          <div
            className="space-y-4 overflow-y-auto px-5 pb-10 pt-2"
            style={{ paddingBottom: "max(2.5rem, env(safe-area-inset-bottom, 2.5rem))" }}
          >
            {/* Gestiona suscripción shortcut */}
            <button
              onClick={handleOpenSubscription}
              className="group flex w-full items-center gap-3 rounded-2xl border border-sky-200/70 bg-gradient-to-r from-sky-50 to-white p-4 text-left transition hover:border-sky-300 hover:shadow-sm"
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-slate-900">Gestiona suscripción</div>
                <div className="text-xs text-slate-500">Estado, facturas y pago</div>
              </div>
              <ChevronRight className="h-4 w-4 flex-shrink-0 text-slate-400 transition group-hover:translate-x-0.5 group-hover:text-sky-600" />
            </button>
      ...
      <AlertDialog open={showLogoutDialog} onOpenChange={setShowLogoutDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("dialog.confirmLogoutDescription")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
