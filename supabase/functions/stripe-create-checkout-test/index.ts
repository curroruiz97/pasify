// [TEST MODE] Copia di stripe-create-checkout con credenziali Stripe TEST.
// Usa secret separati: STRIPE_SECRET_KEY_TEST, STRIPE_PRICE_PARTNER_TEST.
// Elimina questa function quando hai finito il test.
...
const PRICE_ID = Deno.env.get("STRIPE_PRICE_PARTNER_TEST")!;
...
serve(async (req) => {
  ...
  try {
    ...
    // Se trial scaduto o admin grant scaduto, permettiamo il checkout.
    const now = Date.now();
