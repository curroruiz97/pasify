...
CREATE POLICY "organizations_owner_read" ON public.organizations FOR SELECT TO authenticated USING (owner_id = auth.uid());
...
CREATE POLICY "organizations_admin_all" ON public.organizations FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "brands_public_read" ON public.brands FOR SELECT TO anon, authenticated USING (status = 'active');
...
CREATE POLICY "venues_public_read" ON public.venues FOR SELECT TO anon, authenticated USING (status = 'active');
CREATE POLICY "venues_admin_all" ON public.venues FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE TRIGGER trg_events_set_org_chain BEFORE INSERT OR UPDATE OF venue_id ON public.events FOR EACH ROW EXECUTE FUNCTION public.events_set_org_chain();
...
CREATE POLICY "partner_galleries_public_read" ON public.partner_galleries FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY "partner_galleries_admin_all" ON public.partner_galleries FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
