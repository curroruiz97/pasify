-- ============================================================================
-- Pasify · 0014 refund_requests + refund_request_messages
-- Cola de reembolsos con diálogo cliente↔partner↔admin.
-- ============================================================================

CREATE TYPE public.refund_request_status_t AS ENUM (
  'pending',     -- solicitada, esperando decisión partner/admin
  'approved',    -- aprobada, esperando proceso en Stripe
  'rejected',    -- rechazada
  'processing',  -- procesándose en Stripe
  'refunded',    -- reembolsada
  'failed'       -- falló en Stripe (acción manual)
);
...
CREATE TABLE IF NOT EXISTS public.refund_requests (
  ...
  reason_code TEXT,                                       -- categoría: cant_attend|illness|event_change|other
  ...
  stripe_refund_status TEXT,                              -- succeeded|pending|failed
  stripe_failure_reason TEXT,
  processed_at TIMESTAMPTZ,

  -- Auto-approve metadata
  auto_approved BOOLEAN NOT NULL DEFAULT FALSE,
  auto_approve_reason TEXT,                               -- "within_policy_t-7d", "duplicate_request"
...
-- Cliente puede crear solicitud sobre tickets propios
DROP POLICY IF EXISTS "refund_requests_requester_insert" ON public.refund_requests;
CREATE POLICY "refund_requests_requester_insert"
...
-- ============================================================================
-- refund_request_messages — diálogo
-- ============================================================================
...
-- Cliente solicitante puede leer/escribir en su request
DROP POLICY IF EXISTS "refund_messages_requester_read" ON public.refund_request_messages;
CREATE POLICY "refund_messages_requester_read"
  ON public.refund_request_messages FOR SELECT
  TO authenticated
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "refund_messages_requester_insert" ON public.refund_request_messages;
CREATE POLICY "refund_messages_requester_insert"
  ON public.refund_request_messages FOR INSERT
  TO authenticated
  WITH CHECK (
    sender_kind = 'client'
...
DROP POLICY IF EXISTS "refund_messages_member_insert" ON public.refund_request_messages;
CREATE POLICY "refund_messages_member_insert"
  ON public.refund_request_messages FOR INSERT
  TO authenticated
  WITH CHECK (
    sender_kind = 'partner'
...
DROP POLICY IF EXISTS "refund_messages_admin_all" ON public.refund_request_messages;
CREATE POLICY "refund_messages_admin_all"
  ON public.refund_request_messages FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

-- ============================================================================
-- RPC: request_refund(ticket_id, reason, reason_code)
-- ============================================================================
...
BEGIN
  ...
  IF v_ticket.status <> 'paid' THEN
    RAISE EXCEPTION 'Solo tickets pagados pueden reembolsarse (status=%)', v_ticket.status;
  END IF;

  IF v_ticket.used_at IS NOT NULL THEN
    RAISE EXCEPTION 'Ticket ya escaneado, no se puede reembolsar';
  END IF;

  -- Verificar duplicado
  IF EXISTS (SELECT 1 FROM public.refund_requests WHERE ticket_id = _ticket_id AND status NOT IN ('rejected','failed')) THEN
    RAISE EXCEPTION 'Ya existe una solicitud de reembolso para este ticket';
  END IF;

  SELECT * INTO v_event FROM public.events WHERE id = v_ticket.event_id;

  -- Determinar política de auto-aprobación: si la solicitud llega antes del threshold del tier, auto-approve
  ...
  INSERT INTO public.refund_requests (
    ticket_id, order_id, event_id, org_id,
    requester_user_id, requester_email,
    amount_cents, currency, reason, reason_code,
    status, auto_approved, auto_approve_reason,
    decided_at, decided_by
  )
  VALUES (
    _ticket_id, v_ticket.order_id, v_ticket.event_id, v_event.org_id,
    v_uid, v_email,
    v_ticket.amount_paid_cents, v_ticket.currency, _reason, _reason_code,
    ...
    CASE WHEN v_auto_approve THEN v_uid ELSE NULL END  -- system, pero no admin
  )
...
-- ============================================================================
-- RPC: decide_refund(request_id, decision, note)
-- ============================================================================
...
BEGIN
  ...
  IF v_request.id IS NULL THEN
    RAISE EXCEPTION 'Request no encontrada';
  END IF;

  IF v_request.status <> 'pending' THEN
    RAISE EXCEPTION 'Request ya decidida (status=%)', v_request.status;
  END IF;

  -- Permisos: admin platform o owner/admin/manager del org
  IF NOT (
    public.has_role(v_uid, 'admin')
    OR (v_request.org_id IS NOT NULL AND public.has_org_role(v_request.org_id, ARRAY['owner','admin','manager']::public.org_member_role_t[]))
  ) THEN
    RAISE EXCEPTION 'Sin permisos';
  END IF;

  IF _decision NOT IN ('approve','reject') THEN
    RAISE EXCEPTION 'Decision inválida (approve|reject)';
  END IF;

  UPDATE public.refund_requests
  SET status = CASE WHEN _decision = 'approve' THEN 'approved'::public.refund_request_status_t
                    ELSE 'rejected'::public.refund_request_status_t END,
      decided_by = v_uid,
      decided_at = now(),
      decision_note = _note,
      updated_at = now()
