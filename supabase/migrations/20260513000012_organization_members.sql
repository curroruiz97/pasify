-- ============================================================================
-- Pasify · 0012 organization_members + sub-roles + RLS basada en membership
-- ============================================================================

-- ============================================================================
-- enum sub-role del team partner
-- ============================================================================
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname='org_member_role_t') THEN
    CREATE TYPE public.org_member_role_t AS ENUM (
      'owner',       -- dueño legal, control total
      'admin',       -- nivel manager senior
      'manager',     -- gestión operativa (eventos, equipo, finanzas read)
      'rrpp',        -- comerciales / RRPP (crear listas invitados, ver eventos)
      'door_staff',  -- staff puerta (escanear tickets, ver door scans)
      'pos_staff',   -- staff barra/TPV (vender en TPV, top-up cashless)
      'read_only'    -- analista / observador
    );
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.organization_members (
  ...
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,  -- null si pendiente acceptance

  email TEXT NOT NULL,                                    -- email invitado (puede no tener cuenta)
  role public.org_member_role_t NOT NULL,
  status TEXT NOT NULL DEFAULT 'pending'                  -- pending|active|suspended|removed
    CHECK (status IN ('pending','active','suspended','removed')),

  -- Scope opcional: venue/brand específicos. NULL = todos.
...
-- ============================================================================
-- Helper functions: is_member_of(org_id) / has_org_role(org_id, role)
-- ============================================================================
...
DROP POLICY IF EXISTS "org_members_org_read" ON public.organization_members;
CREATE POLICY "org_members_org_read"
  ON public.organization_members FOR SELECT
  TO authenticated
  USING (
    public.has_org_role(org_id, ARRAY['owner','admin','manager']::public.org_member_role_t[])
  );

DROP POLICY IF EXISTS "org_members_owner_admin_write" ON public.organization_members;
CREATE POLICY "org_members_owner_admin_write"
  ON public.organization_members FOR ALL
  TO authenticated
  USING (
    public.has_org_role(org_id, ARRAY['owner','admin']::public.org_member_role_t[])
  )
  WITH CHECK (
    public.has_org_role(org_id, ARRAY['owner','admin']::public.org_member_role_t[])
  );

DROP POLICY IF EXISTS "org_members_platform_admin_all" ON public.organization_members;
CREATE POLICY "org_members_platform_admin_all"
  ON public.organization_members FOR ALL
  TO authenticated
...
-- ============================================================================
-- Reforzar policies de tablas multi-tenant con membership
-- ============================================================================

-- organizations: miembros activos pueden ver
...
-- brands: miembros activos pueden ver/escribir según rol
...
DROP POLICY IF EXISTS "brands_member_write" ON public.brands;
CREATE POLICY "brands_member_write"
...
-- venues: miembros activos pueden ver/escribir según rol
DROP POLICY IF EXISTS "venues_member_read" ON public.venues;
CREATE POLICY "venues_member_read"
  ON public.venues FOR SELECT
  TO authenticated
  USING (public.is_member_of_org(org_id));

DROP POLICY IF EXISTS "venues_member_write" ON public.venues;
CREATE POLICY "venues_member_write"
  ON public.venues FOR ALL
  TO authenticated
  USING (public.has_org_role(org_id, ARRAY['owner','admin','manager']::public.org_member_role_t[]))
  WITH CHECK (public.has_org_role(org_id, ARRAY['owner','admin','manager']::public.org_member_role_t[]));

-- events: refrescamos policies para usar membership (venue_id)
...
CREATE POLICY "events_member_read"
  ...
  USING (
    partner_id = auth.uid()
    OR (org_id IS NOT NULL AND public.is_member_of_org(org_id))
  );
...
CREATE POLICY "events_member_insert"
  ...
  WITH CHECK (
    partner_id = auth.uid()
    OR (org_id IS NOT NULL AND public.has_org_role(org_id, ARRAY['owner','admin','manager','rrpp']::public.org_member_role_t[]))
  );
...
CREATE POLICY "events_member_update"
  ...
  USING (
    partner_id = auth.uid()
    OR (org_id IS NOT NULL AND public.has_org_role(org_id, ARRAY['owner','admin','manager']::public.org_member_role_t[]))
  )
  WITH CHECK (
    partner_id = auth.uid()
    OR (org_id IS NOT NULL AND public.has_org_role(org_id, ARRAY['owner','admin','manager']::public.org_member_role_t[]))
...
CREATE POLICY "events_member_delete"
  ...
  USING (
    partner_id = auth.uid()
    OR (org_id IS NOT NULL AND public.has_org_role(org_id, ARRAY['owner','admin']::public.org_member_role_t[]))
  );

-- tickets: partners marcan como used vía door_staff o roles superiores
DROP POLICY IF EXISTS "tickets_partner_read_own_events" ON public.tickets;
CREATE POLICY "tickets_member_read"
    ...
    EXISTS (
      ...
      WHERE e.id = tickets.event_id
        AND (
          ...
          OR (e.org_id IS NOT NULL AND public.is_member_of_org(e.org_id))
        )
    )
  );
...
-- ============================================================================
-- RPC: invite_member(org_id, email, role, venue_id, brand_id)
-- ============================================================================
CREATE OR REPLACE FUNCTION public.invite_member(
  _org_id UUID,
  _email TEXT,
  _role public.org_member_role_t,
...
BEGIN
  ...
  -- Permisos: solo owner/admin de la org pueden invitar
  IF NOT public.has_org_role(_org_id, ARRAY['owner','admin']::public.org_member_role_t[]) THEN
    RAISE EXCEPTION 'Insufficient permissions';
  END IF;

  -- Buscar si el email ya tiene cuenta Pasify
  SELECT id INTO v_existing_user FROM public.profiles WHERE email = _email LIMIT 1;

  -- Upsert por (org_id, email)
  INSERT INTO public.organization_members (
    org_id, user_id, email, role, venue_id, brand_id,
    invited_by, invitation_expires_at, status
  )
  VALUES (
    _org_id, v_existing_user, _email, _role, _venue_id, _brand_id,
...
-- ============================================================================
-- RPC: accept_invitation(token) — usuario autenticado acepta su invitación
-- ============================================================================
...
BEGIN
  ...
  IF v_member_row.id IS NULL THEN
...
-- ============================================================================
-- RPC: tenant_for_user() — devuelve la org/brand/venue por defecto del user
-- ============================================================================
...
AS $$
  WITH ctx AS (
    SELECT
      p.id AS user_id,
      p.last_active_venue_id
    FROM public.profiles p
    WHERE p.id = auth.uid()
    LIMIT 1
  ),
  membership AS (
    -- Owner toma precedencia
  ...
  SELECT
    ...
    COALESCE(c.last_active_venue_id, v.id) AS venue_id,
    COALESCE(av.name, v.name) AS venue_name,
    pm.role
